...
Keep the eight-line composition as the baseline. The centre is readable without adding another runtime layer, and the outer branches retain their route colours and aggregate movement. The first expansion, [Métro 2 and 6](METRO-ARCS.md), now forms an optional ten-line study with separate payload, visual and frame checks. The second group, [Métro 5 and 7](METRO-CROSSINGS.md), passes those checks with both optional groups enabled, forming a twelve-line study. The third group, [Métro 3 and 11](METRO-EAST.md), extends the optional composition to fourteen lines under the same review gates. The [complete-network review](METRO-COMPLETE.md) adds the final seven Métro lines and checks all 21 Métro/RER lines together. The [Transilien review](TRANSILIEN.md) extends this to 30 optional rail lines. The [first tram review](TRAM.md) adds T3a/T3b. Remaining tram groups and operational rail variation remain later studies.
