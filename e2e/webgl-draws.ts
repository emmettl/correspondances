...
export function installWebGLDrawCounters() {
  window.parisDraws = { draws: 0, zeroOpacityDraws: 0 }
  ...
  function count(gl: WebGL2RenderingContext, vertices: number) {
    ...
    window.parisDraws.draws++
    const program = programs.get(gl)
    if (program && opacities.get(program) === 0) window.parisDraws.zeroOpacityDraws++
  }
