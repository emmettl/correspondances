...
for (const spec of [
  ...
  assert.deepEqual([...new Set(morning.data.trains.map((train) => train.route))].sort(), spec.routes)
...
assert.equal(merged.trains.length, 1516)
assert.equal(new Set(merged.trains.map((train) => train.route)).size, 12)
assert.equal(merged.trains.filter((train) => train.start <= 28_800 && train.end >= 28_800).length, 482)
console.log('Twelve-line composition verified: 1516 morning journeys and 482 vehicles active at 08:00.')
