...
const DEFAULT_BOUNDS = [1.5, 48.3, 3.4, 49.35]
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === '--input') options.inputs.push(argv[++index])
      ...
      console.log('Usage: node scripts/ingest-paris-air.mjs (--input 09.bin.ttf ... | --input-directory /path/to/slices) --window-start 06:45 --window-end 08:45 [--chunk-hours 1] [--output file]')
      process.exit(0)
...
async function writeChunkedArtifacts(options, tracks) {
  ...
  for (
    ...
    await writeFile(join(outputDirectory, path), payload)
...
async function main() {
  ...
  for (const input of options.inputs) {
    ...
    const sourceDate = /^\d{4}-\d{2}-\d{2}/.exec(name)?.[0] ?? options.serviceDate
    options.sourceFiles.push({
      name,
      sha256: createHash('sha256').update(compressed).digest('hex'),
      sourceUrl: `https://github.com/adsblol/globe_history_${sourceDate.slice(0, 4)}/releases/tag/v${sourceDate.replaceAll('-', '.')}-planes-readsb-prod-0`,
    })
