...
export function useParisMetroArcs(edition: ParisEdition, window: 'morning' | 'day', time: number) {
  ...
  const day = useProgressiveNetworkDay(
    edition.data.layers.metroArcsDayManifest + (attempt ? `?retry=${attempt}` : ''),
  ...
  useEffect(() => {
    ...
    void fetch(editionDataUrl(edition.data.layers.metroArcsMorning), { signal: controller.signal })
      .then(async (response) => {
        if (!response.ok) throw new Error('Paris Métro arcs unavailable')
        ...
        if (snapshot.metadata.serviceDate !== '2026-09-04' ||
            ...
            !Array.isArray(snapshot.trains)) throw new Error('Paris Métro arcs do not match the study')
  ...
  }, [enabled, window, morning, edition.data.layers.metroArcsMorning, attempt])
