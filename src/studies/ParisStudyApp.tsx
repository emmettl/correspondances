  ...
  return (
    ...
    >
      ...
      {selectedAirport ? (
        ...
        status={`Mission ${selectedTrain.shortName}`} atStopLabel="À l’arrêt" labels={{rail:'Train',bus:'Bus',destination:'Destination',destinationUnknown:'Destination indisponible',nextStop:'Prochain arrêt',callingAt:'Arrêts suivants',terminus:'Terminus',platform:'Voie',empty:'Aucun autre arrêt dans les données chargées.'}}
        note="Horaires planifiés · données chargées · pas de temps réel" /></Suspense></div> : selectedStation && network && !selectedConnection ? <div className="edition-station-hero ms-study-panel"><div className="paris-status station-hero-summary"><strong>{activeTrainCount}</strong><span>trains en mouvement</span><span>{selectedStation.name}</span></div><Suspense fallback={null}><StationDeparturesCard snapshot={network} name={selectedStation.name} time={time}
